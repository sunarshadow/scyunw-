...
class Partner extends Common
{
    ...
    public function GetSet(){
		...
		if(!$id){
			...
			if($info){$jsonary["msg"] = "合作伙伴名称已存在，请重新输入";}
		}
		//执行操作
		if(!$jsonary["msg"]){
			if($id>0){//修改
				$result = model($this->table)->where('id', $id)->update($post);
				model("Base")->CreateAdminLog("编辑合作伙伴信息","修改合作伙伴[".$post["title"]."]的信息");
			}else{//增加
				$post["addtime"] = $post["lastedit"] = date("Y-m-d H:i:s");
				$result = model($this->table)->insert($post);
				model("Base")->CreateAdminLog("添加合作伙伴信息","添加合作伙伴[".$post["title"]."]的信息");
			}
		}
		return json($jsonary);
	}
	...
	public function GetDel(){
		...
		model("Base")->CreateAdminLog("删除合作伙伴","删除合作伙伴，ID[".$ids."]");
		$jsonary["code"] = $result;
		return json($jsonary);
	}
